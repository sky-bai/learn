...
type Item struct { // 传入参数
	...
	// SetNX only sets the key if it does not already exist. 不存在的key 才设置
	SetNX bool
...
// Options is a cache options. 可选项
type Options struct {
	Redis        rediser       // 可以设置的redis方法接口
	LocalCache   LocalBigCache // 可以设置的本地缓存接口
	StatsEnabled bool
	Marshal      MarshalFunc
...
type Cache struct {
	...
	group singleflight.Group // 单飞库的使用
...
// Set caches the item. 为什么要包一层呢？ 1.todo
...
func (cd *Cache) getBytes(ctx context.Context, key string, skipLocalCache bool) ([]byte, error) {
	if !skipLocalCache && cd.opt.LocalCache != nil {
		b, err := cd.opt.LocalCache.Get(key)
		if err != nil {
			return nil, err
		} else {
			return b, nil
...
func (cd *Cache) Once(item *Item) error { // 确保只有一次
...
func (cd *Cache) getSetItemBytesOnce(item *Item) (b []byte, cached bool, err error) {
	...
	// 本地拿没有拿到，防止多次请求打到redis上 do 就是放实际操作获取资源数据的方法
...
// 什么时候该用本地缓存 为什么redis不够用了
// 1.解决了什么问题 todo
// 看看操作步骤 set get delete

// 本地缓存+redis缓存 本地缓存是为了解决redis缓存的热点问题  什么是热点问题 什么是热点key todo
// 对于热点key就直接走本地缓存了
// 使用lfu算法来实现本地缓存结构体
...
// 什么时候该只删除本地缓存，什么时候该删除redis缓存呢？
// bigCache的实现原理
// 本地缓存利器bigCache 和 redis的缓存的组装使用
