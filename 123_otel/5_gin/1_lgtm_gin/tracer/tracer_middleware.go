...
func Extract(req *http.Request, spanName string) Span {
	// TextMapPropagator 接口定义了一种在文本映射（如HTTP请求标头或HTTP请求参数）中编码和解码跟踪上下文信息的方法。
	// 不同的otel后端和tracer实现可以使用不同的文本格式来表示跟踪信息，但通过实现这个接口，你可以确保在不同的组件之间传递跟踪上下文信息时保持一致性。
	var propagator = otel.GetTextMapPropagator()
	// 1.获取编解码器
	pctx := propagator.Extract(req.Context(), propagation.HeaderCarrier(req.Header))
	// 2.从header中获取trace-id和span-id
